...
async fn users_status() {
    ...
    let options = UserStatusOptions::new().signal(true).game_ids(false).game_metas(true);

    // Run some test cases
    let result = LI.users().status(&["adriana", "ana", "bot0"], None).await;
    assert!(result.is_ok(), "Failed to get user statuses: {:?}", result.unwrap_err().source().unwrap());
...
async fn users_top10() {
    ...
    assert!(result.is_ok(), "Failed to get top 10 lists: {:?}", result.unwrap_err().source().unwrap());
...
async fn users_leaderboard() {
    ...
    assert!(result.is_ok(), "Failed to get leaderboard: {:?}", result.unwrap_err().source().unwrap());
...
async fn users_profile() {
    ...
    assert!(result.is_ok(), "Failed to get user profile: {:?}", result.unwrap_err().source().unwrap());
...
async fn users_rating_history() {
    ...
    assert!(result.is_err(), "Getting user rating history did not fail: {:?}", result.unwrap());
}
...
async fn users_performance() {
    ...
    assert!(result.is_err(), "Getting user performance statistics did not fail: {:?}", result.unwrap());
}
...
async fn users_list() {
    ...
    assert!(result.is_ok(), "Failed to get list of users: {:?}", result.unwrap_err().source().unwrap());
...
async fn users_notes_write() {
    ...
    let result = LI.users().notes_write("Adriana", "This is a private test note").await;
    assert!(
        result.is_ok(),
        "Failed to write to private notes: {:?}",
        result.unwrap_err().source().unwrap()
    ...
    let result = LI.users().notes_write("NoSuchUser", "This is a private test note").await;
    assert!(result.is_err(), "Writing to private notes did not fail: {:?}", result.unwrap());

    let result = DEFAULT.users().notes_write("Bot0", "This is a private test note").await;
    assert!(result.is_err(), "Writing to private notes did not fail: {:?}", result.unwrap());
}
