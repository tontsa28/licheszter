...
async fn puzzle_daily() {
    ...
    assert!(result.is_ok(), "Failed to get daily puzzle: {:?}", result.unwrap_err().source().unwrap());
...
async fn puzzle_show() {
    ...
    assert!(result.is_ok(), "Failed to get puzzle: {:?}", result.unwrap_err().source().unwrap());
...
async fn puzzle_next() {
    ...
    assert!(result.is_ok(), "Failed to get next puzzle: {:?}", result.unwrap_err().source().unwrap());

    let result = LI.puzzles().next(Some("rookEndgame"), Some(PuzzleDifficulty::Normal)).await;
    assert!(result.is_ok(), "Failed to get next puzzle: {:?}", result.unwrap_err().source().unwrap());
...
async fn puzzle_activity() {
    ...
    while let Some(event) = result.next().await {
        assert!(event.is_ok(), "Failed to get puzzle activity: {:?}", event.unwrap_err().source().unwrap());
    }
